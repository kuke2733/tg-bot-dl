...
把机器人加进频道（需要管理员权限），然后发送：

```text
/listen @频道用户名
```

- 新帖子里的媒体会自动下载到 `data/频道标题/` 文件夹；相册照旧在频道文件夹里再建子文件夹。
...
- 下载超过 20MiB 的文件需要用户账号（`PHONE_NUMBER`）也加入该频道。
- `/listening` 查看列表，`/unlisten` 停止监听。跳过原因都会写进摘要和日志，方便调整过滤规则。
