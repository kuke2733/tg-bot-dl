...
- 🗂 **队列与文件管理**：`/queue` 查看并取消任务、`/pause` `/resume` 暂停恢复、`/files` 浏览和删除文件；任务实时持久化，进程重启自动恢复断点续传。
...
3. **Telegram 手机号**（频道监听必填，下载强烈建议配置）：用户账号承担实际的文件下载和频道监听，机器人只负责交互。不配置则无法使用频道监听。
...
| `/listen 频道` | 监听频道，新帖自动下载到 `data/频道标题/` 文件夹（含广告过滤） |
...
- [ ] **断点续传**：~~进程内失败续传~~、~~进程重启后恢复~~（已支持，配合队列持久化）；仍缺：手动停止后保留
